module.exports = (path, allowPure) => {
  const checkSuperClassIdentifier = (p) => {
    if (p.isIdentifier({ name: 'Component' })) {
      return 'Component';
    }
    if (allowPure && p.isIdentifier({ name: 'PureComponent' })) {
  ...
  if (binding.path.isVariableDeclarator()) {
    ...
    if (searchIdentifier !== superClass) {
      ...
      //       ^^^^^
      return id.isIdentifier() ? { type, import: id } : null;
    }
