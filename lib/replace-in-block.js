module.exports = (path, id, fn, statics, t) => {
  ...
  if (ancestorPath.isExportNamedDeclaration()) {
    ancestorPath.replaceWith(t.exportNamedDeclaration(null, [t.exportSpecifier(id, id)]));
  } else {
    path.replaceWith(id);
  }
  ...
  ancestorPath.insertBefore([
    ...
    ...statics.map(({ key, value }) =>
      t.expressionStatement(t.assignmentExpression('=', t.MemberExpression(id, key), value)),
    ),
  ]);
